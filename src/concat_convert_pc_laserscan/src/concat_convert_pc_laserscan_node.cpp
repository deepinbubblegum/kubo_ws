...
sensor_msgs::LaserScanPtr pointcloud_to_laserscan(sensor_msgs::PointCloud2 *merged_cloud)
{
    ...
    output->angle_min = min_ang_;
    // -179 degrees
    output->angle_max = max_ang_;
    // +179 degrees
...
void concat_with_pc()
{
    while (ros::ok())
    {
        ...
        sharedPtr = ros::topic::waitForMessage<sensor_msgs::PointCloud2>("/front/rslidar_packets", ros::Duration(1));
        if (sharedPtr == NULL)
        ...
        sharedPtr = ros::topic::waitForMessage<sensor_msgs::PointCloud2>("/back/rslidar_packets", ros::Duration(1));
        if (sharedPtr == NULL)
            ROS_INFO("No laser messages received");
        else
            cloud_2 = *sharedPtr;
